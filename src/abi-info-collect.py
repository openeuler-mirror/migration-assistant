...
def find_soname_file(f_elf, f_ldd):
    ...
    with open(f'{f_ldd}', 'rt') as f:
        for line in f:
            for sym in sym_set:
                # libcrypto.so.10 => /lib64/libcrypto.so.10 (0x00007f9327355000)
                if not re.match(' *(?P<name>[a-zA-Z]*).*\).*=>.*', line):
                    continue
                # libdl.so.2 (GLIBC_2.2.5) => /lib64/libdl.so.2
                if not re.search(f'({sym})', line):
                    continue
                soname_file_set.add(line.strip().split("=>")[1])
